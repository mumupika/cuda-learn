...
int main() {
    ...
    cudaMemcpy2DToArray(cuArray, 0, 0, host, spitch, W * sizeof(float),
                        H, cudaMemcpyHostToDevice);
    
    float *host_res = (float *) malloc(sizeof(float) * 3 * 3);
    cudaMemcpy2DFromArray(host_res, sizeof(float) * 3, cuArray, sizeof(float) * 2, 2, sizeof(float) * 3, 3, cudaMemcpyDeviceToHost);
    for(int i = 0; i < 9; i++) {
        printf("%f ", host_res[i]);
    }
    printf("\n");
